...
class ErrorTests(unittest.TestCase):
    def setUp(self):
        ...
        self.ratio = pd.DataFrame(
            {
                "standard error": {
                    0: 0.66458146620536607,
                    1: 0.75702195760340563,
                    2: 0.71587945049229107,
                    3: 0.68021031918966046,
                    4: 0.16281610758487003,
                },
                "optimal block": {0: "", 1: "", 2: "", 3: "<---    ", 4: ""},
                "mean": {
                    0: -0.086689548180912318,
                    1: -0.086689548180912304,
                    2: -0.086689548180912318,
                    3: -0.08668954818091229,
                    4: -0.08668954818091229,
                },
            }
        )
    ...
    def test_ratio(self):
        ratio = pyblock.error.ratio(
            self.reblock[1], self.reblock[2], self.cov_12, self.data_len
        )
    ...
    def test_ratio_single(self):
        ratio = pyblock.error.ratio(
            self.reblock[1].loc[4],
            self.reblock[2].loc[4],
            self.cov_12[4],
            self.data_len[4],
        )
    ...
    def test_product_single(self):
        product = pyblock.error.product(
            self.reblock[1].iloc[4],
            self.reblock[2].iloc[4],
            self.cov_12[4],
            self.data_len[4],
        )
    ...
    def test_subtraction_single(self):
        subtraction = pyblock.error.subtraction(
            self.reblock[1].iloc[4],
            self.reblock[2].iloc[4],
            self.cov_12[4],
            self.data_len[4],
    ...
    def test_addition_single(self):
        addition = pyblock.error.addition(
            self.reblock[1].iloc[4],
            self.reblock[2].iloc[4],
            self.cov_12[4],
            self.data_len[4],
...
class ErrorFmtTests(unittest.TestCase):
    ...
    def test3(self):
        self.assertEqual(
            pyblock.error.pretty_fmt_err(float("inf"), 100), str(float("inf"))
        )

    def test4(self):
        self.assertEqual(
            pyblock.error.pretty_fmt_err(100.01, float("inf")),
            "100.01(%s)" % (float("inf")),
        )
